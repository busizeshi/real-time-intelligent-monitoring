...
void *drm_buf_alloc_fix(drm_context *drm_ctx, int drm_fd, int TexWidth, int TexHeight, int bpp, int *fd, unsigned int *handle, size_t *actual_size)
{
    int ret;
    if (drm_ctx == NULL)
    {
    ...
    // todo
    if (drm_ctx == NULL || drm_ctx->io_func == NULL) {
        printf("drm_ctx or io_func is NULL\n");
        return NULL;
    }
...
int drm_buf_destroy_fix(drm_context *drm_ctx, int drm_fd, int buf_fd, int handle, void *drm_buf, size_t size)
{
    int ret = -1;
    if (drm_buf == NULL)
    {
